...
def extract_harmony_streaming_delta(
    ...
    # Check for constraint recipients first - they should be treated as function calls
    # even when on the "final" channel (GPT-OSS outputs JSON constraints this way)
    is_constraint_recipient = (
        cur_recipient
        and (cur_recipient.startswith("functions.") or cur_recipient == "<|constrain|>json" or cur_recipient == "<|channel|>commentary")
    )

    if cur_channel == "final" and not is_constraint_recipient:
        delta_message = DeltaMessage(content=delta_text)
    elif is_constraint_recipient:
        ...
        if prev_recipient != cur_recipient:
            tool_name = cur_recipient.split("functions.", 1)[1] if cur_recipient.startswith("functions.") else "final_result"
