...
class OpenAIServingResponses(OpenAIServing):
        ...
        try:
            for engine_prompt in engine_prompts:
                ...
                if self.reasoning_parser is not None:
                    ...
                    if envs.VLLM_SKIP_JSON_CONSTRAINT and struct_out.json is not None:
                        from dataclasses import replace
                        sampling_params.structured_outputs = replace(struct_out, json=None)
                        struct_out = sampling_params.structured_outputs
    ...
    def _emit_function_call_done_events(
        ...
        if previous_item.recipient.startswith("functions."):
            ...
            function_name = "final_result"
        events = []
        emit_delta_in_done = False  # Track if we need to emit delta here
        
        # If delta path was skipped (e.g., parser buffered content for constraint validation),
        # emit the Added event here before Done
        if state.is_first_function_call_delta is False:
            ...
            emit_delta_in_done = True  # Only emit delta if we also emitted Added
        ...
        # Only emit delta if streaming path was skipped (we emitted Added above)
        if emit_delta_in_done:
            events.append(
                ResponseFunctionCallArgumentsDeltaEvent(
                    item_id=state.current_item_id,
                    delta=previous_item.content[0].text,
                    output_index=state.current_output_index,
                    sequence_number=-1,
    ...
    def _emit_previous_item_done_events(
        ...
        if previous_item.recipient is not None:
            # Deal with tool call - including constraint recipients from channel=final
            if (
                previous_item.recipient.startswith("functions.")
                or previous_item.recipient == "<|constrain|>json"
                or previous_item.recipient == "<|channel|>commentary"
                or previous_item.recipient.startswith("<|constrain|>")  # Handle any constraint recipient
            ):
        ...
        # Handle channel='final_result' as function call (GPT-OSS outputs final_result JSON here)
        elif previous_item.channel == "final_result":
            return self._emit_function_call_done_events(previous_item, state)
        elif previous_item.channel == "analysis":
            return self._emit_reasoning_done_events(previous_item, state)
        elif previous_item.channel == "final":
            # Check if content looks like final_result JSON (GPT-OSS sometimes outputs without recipient)
            content = "".join(c.text for c in previous_item.content if hasattr(c, 'text'))
            content_stripped = content.strip()
            if content_stripped.startswith("{") and '"r"' in content_stripped:
                # This looks like final_result JSON - treat as function call
                return self._emit_function_call_done_events(previous_item, state)
    ...
    def _emit_content_delta_events(
        ...
        import sys
        print(f"DEBUG _emit_delta: channel={ctx.parser.current_channel!r}, recipient={ctx.parser.current_recipient!r}, delta={ctx.last_content_delta!r:.50}", file=sys.stderr, flush=True)
        if not ctx.last_content_delta:
            print(f"DEBUG _emit_delta: EMPTY DELTA, returning early", file=sys.stderr, flush=True)
            return []

        # Handle channel='final_result' as function call (GPT-OSS outputs final_result JSON here)
        if ctx.parser.current_channel == "final_result":
            return self._emit_function_call_delta_events(ctx, state)
        # Plain text on final channel
        elif (
            ctx.parser.current_channel == "final"
            and ctx.parser.current_recipient is None
        ):
            return self._emit_final_channel_delta_events(ctx, state)
        # Handle channel='final' with constraint/special recipients as function call (for GPT-OSS)
        elif (
            ctx.parser.current_channel == "final"
            and ctx.parser.current_recipient is not None
            and (
                ctx.parser.current_recipient.startswith("<|constrain|>")
                or ctx.parser.current_recipient == "<|channel|>commentary"
                or ctx.parser.current_recipient.startswith("functions.")
            )
        ):
            return self._emit_function_call_delta_events(ctx, state)
        elif (
    ...
    def _emit_function_call_delta_events(
        ...
        import sys
        print(f"DEBUG _emit_fc_delta: channel={ctx.parser.current_channel!r}, recipient={ctx.parser.current_recipient!r}", file=sys.stderr, flush=True)
        # Check if this is a function call - handle regardless of channel
        # (GPT-OSS can output function calls on the "final" channel)
        is_function_call = (
            ctx.parser.current_recipient
            and (
                ctx.parser.current_recipient.startswith("functions.")
                or ctx.parser.current_recipient == "<|constrain|>json"
                or ctx.parser.current_recipient == "<|channel|>commentary"
                or ctx.parser.current_recipient.startswith("<|constrain|>")
            )
        )
        if not is_function_call:
            print(f"DEBUG _emit_fc_delta: RETURNING EARLY", file=sys.stderr, flush=True)
            return []
        ...
        if state.is_first_function_call_delta is False:
            ...
            elif ctx.parser.current_recipient.startswith("<|constrain|>"):
                fc_name = "final_result"  # Map any constraint recipient to final_result
            else:
                fc_name = "final_result"  # Default fallback
            state.current_item_id = f"fc_{random_uuid()}"
            tool_call_item = ResponseFunctionToolCall(
                name=fc_name,
                type="function_call",
